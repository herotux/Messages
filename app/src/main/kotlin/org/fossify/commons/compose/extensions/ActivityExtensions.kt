...
/**
 * Disabled in Homa's Commons fork.
 *
 * Homa is a legitimate Fossify-based fork with its own applicationId, so the
 * upstream package-name heuristic must not show the "fake version" warning.
 */
fun Context.fakeVersionCheck(
    showConfirmationDialog: () -> Unit
) {
    // Intentionally disabled for legitimate Fossify-based forks.
}
