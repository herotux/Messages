...
fun HomaBottomSheet(onDismissRequest: () -> Unit, modifier: Modifier = Modifier, content: @Composable ColumnScope.() -> Unit) {
    ModalBottomSheet(onDismissRequest = onDismissRequest, modifier = modifier, windowInsets = WindowInsets.navigationBars, dragHandle = { BottomSheetDefaults.DragHandle() }, shape = RoundedCornerShape(topStart = HomaDesignTokens.Shape.extraLarge, topEnd = HomaDesignTokens.Shape.extraLarge), containerColor = MaterialTheme.colorScheme.surface) { androidx.compose.foundation.layout.Column(Modifier.fillMaxWidth().navigationBarsPadding().padding(HomaDesignTokens.Spacing.md), content = content) }
}

@Composable
fun HomaDialog(onDismissRequest: () -> Unit, title: String, text: String? = null, confirm: @Composable () -> Unit, dismiss: (@Composable () -> Unit)? = null) {
    AlertDialog(onDismissRequest = onDismissRequest, title = { androidx.compose.material3.Text(title) }, text = text?.let { { androidx.compose.material3.Text(it) } }, confirmButton = confirm, dismissButton = dismiss, shape = RoundedCornerShape(HomaDesignTokens.Shape.extraLarge))
}
