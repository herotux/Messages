...
fun HomaBottomSheet(onDismissRequest: () -> Unit, modifier: Modifier = Modifier, content: @Composable ColumnScope.() -> Unit) {
    ModalBottomSheet(
        ...
        shape = RoundedCornerShape(
            topStart = HomaDesignTokens.Shape.extraLarge,
            topEnd = HomaDesignTokens.Shape.extraLarge,
        ),
        containerColor = MaterialTheme.colorScheme.surface,
    ) {
        Column(
            Modifier.fillMaxWidth().navigationBarsPadding().padding(HomaDesignTokens.Spacing.md),
            content = content,
        )
    }
}
...
@Composable
fun HomaSurface(modifier: Modifier = Modifier, content: @Composable () -> Unit) = Surface(
    modifier = modifier,
