...
class FixedConversationFolderTabsView @JvmOverloads constructor(
    ...
    private fun sync(force: Boolean = false) {
        val enabled = ConversationFolderManager.areFoldersEnabled(context)
        if (!enabled) {
            beGone()
            return
        }
    ...
    private fun wiggle(enabled: Boolean) {
        ...
        for (i in 0 until tabs.childCount) {
            ...
            if (!enabled) {
                ...
                val a = ObjectAnimator.ofFloat(v, View.ROTATION, -1.7f, 1.7f).apply { duration = 105; repeatMode = ValueAnimator.REVERSE; repeatCount = ValueAnimator.INFINITE }
    ...
    private fun reorderByPosition(view: View) {
        ...
        tabs.removeViewAt(a); tabs.addView(view, b); view.translationX = 0f; view.performHapticFeedback(android.view.HapticFeedbackConstants.CLOCK_TICK)
    }
