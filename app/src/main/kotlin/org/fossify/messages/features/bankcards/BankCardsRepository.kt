...
class BankCardsRepository(private val context: Context) {
    ...
    fun save(existing: BankCard?, cardNumber: String, holderName: String, iban: String): Result<Unit> = runCatching {
        ...
        val bank = IranianBankRegistry.findByCard(normalizedCard)
            ?: error("Unknown bank")
    ...
    fun delete(card: BankCard): Result<Unit> = runCatching {
        context.getMessagesDB().BankAccountsDao().delete(
            BankAccount(
                id = card.id,
                bankId = card.bankId,
        ...
        cards.forEachIndexed { index, card ->
            dao.update(
                BankAccount(
                    ...
                    iban = card.iban,
                    createdAt = card.createdAt,
                    updatedAt = System.currentTimeMillis() + cards.size - index
                )
            )
        }
    }
    ...
    fun normalizeIban(value: String): String = normalizeDigits(value)
        .replace(" ", "")
        .replace("-", "")
        .uppercase(Locale.US)
