...
import org.fossify.messages.dialogs.MessageAnnotationDialogs
...
import org.fossify.messages.views.MessageAnnotationsView
...
class ThreadAdapter(activity: SimpleActivity, recyclerView: MyRecyclerView, itemClick: (Any) -> Unit, val isRecycleBin: Boolean, val deleteMessages: (messages: List<Message>, toRecycleBin: Boolean, fromRecycleBin: Boolean) -> Unit) : MyRecyclerViewListAdapter<ThreadItem>(activity, recyclerView, ThreadItemDiffCallback(), itemClick) {
    ...
    companion object { private const val MAX_MEDIA_HEIGHT_RATIO = 3; private const val SIM_BITS = 21; private const val SIM_MASK = (1L shl SIM_BITS) - 1; private const val ANNOTATIONS_TAG = "homa_message_annotations" }
    ...
    private fun editSelectedLabels() { (getSelectedItems().firstOrNull() as? Message)?.let { message -> MessageAnnotationDialogs.editLabels(activity, message) { notifyMessageAnnotationChanged(message.id) } } }
    private fun editSelectedNote() { (getSelectedItems().firstOrNull() as? Message)?.let { message -> MessageAnnotationDialogs.editNote(activity, message) { notifyMessageAnnotationChanged(message.id) } } }
    private fun notifyMessageAnnotationChanged(messageId: Long) { currentList.indexOfFirst { (it as? Message)?.id == messageId }.takeIf { it >= 0 }?.let { notifyItemChanged(it) } }
    ...
    private fun setupAnnotations(holder: ViewHolder, binding: ItemMessageBinding, message: Message) {
        ...
        var view = wrapper.findViewWithTag<MessageAnnotationsView>(ANNOTATIONS_TAG)
        if (view == null) {
            view = MessageAnnotationsView(activity).apply { tag = ANNOTATIONS_TAG }
            wrapper.addView(view, RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT).apply {
                addRule(RelativeLayout.BELOW, binding.threadMessageBody.id)
                addRule(RelativeLayout.ALIGN_START, binding.threadMessageBody.id)
                addRule(RelativeLayout.ALIGN_END, binding.threadMessageBody.id)
                topMargin = (2 * activity.resources.displayMetrics.density).toInt()
                bottomMargin = (2 * activity.resources.displayMetrics.density).toInt()
            })
        }
        view.setTag(ANNOTATIONS_TAG, message.id)
        view.render(emptyList(), null)
        ensureBackgroundThread {
            ...
            activity.runOnUiThread {
                if (!activity.isDestroyed && !activity.isFinishing && view.getTag(ANNOTATIONS_TAG) == message.id) {
                    view.render(labels, note)
                }
            }
        }
    }
    ...
    private fun setupThreadSuccess(view: View, delivered: Boolean) { ItemThreadSuccessBinding.bind(view).apply { threadSuccess.setImageResource(if (delivered) R.drawable.ic_check_double_vector else org.fossify.messages.R.drawable.ic_check_vector); threadSuccess.applyColorFilter(textColor) } }
