...
class ThreadAdapter(activity: SimpleActivity, recyclerView: MyRecyclerView, itemClick: (Any) -> Unit, val isRecycleBin: Boolean, val deleteMessages: (messages: List<Message>, toRecycleBin: Boolean, fromRecycleBin: Boolean) -> Unit) : MyRecyclerViewListAdapter<ThreadItem>(activity, recyclerView, ThreadItemDiffCallback(), itemClick) {
    ...
    private fun askConfirmDelete() { val count = selectedKeys.size; val items = try { resources.getQuantityString(R.plurals.delete_messages, count, count) } catch (e: Exception) { activity.showErrorToast(e); return }; val base = if (activity.config.useRecycleBin && !isRecycleBin) org.fossify.messages.R.string.move_to_recycle_bin_confirmation else org.fossify.messages.R.string.deletion_confirmation; val question = String.format(resources.getString(base), items); DeleteConfirmationDialog(activity, question, activity.config.useRecycleBin && !isRecycleBin) { skip -> ensureBackgroundThread { val messages = getSelectedItems(); if (messages.isNotEmpty()) deleteMessages(messages.filterIsInstance<Message>(), !skip && activity.config.useRecycleBin && !isRecycleBin, false) } } }
    ...
    private fun setupDateTime(view: View, dateTime: ThreadDateTime) { ItemThreadDateTimeBinding.bind(view).apply { threadDateTime.apply { text = if (activity.config.usePersianCalendar) PersianDateHelper.toPersianDigits(PersianDateHelper.formatMonthName(dateTime.date * 1000L)) else (dateTime.date * 1000L).formatDateOrTime(context = context, hideTimeOnOtherDays = false, showCurrentYear = false); setTextSize(TypedValue.COMPLEX_UNIT_PX, fontSize) }; val colors = conversationTheme()?.colors; threadDateTime.setTextColor(colors?.textSecondary ?: textColor); threadSimIcon.beVisibleIf(hasMultipleSIMCards); threadSimNumber.beVisibleIf(hasMultipleSIMCards); if (hasMultipleSIMCards) { threadSimNumber.text = dateTime.simID; threadSimNumber.setTextColor((colors?.textSecondary ?: textColor).getContrastColor()); threadSimIcon.applyColorFilter(colors?.accent ?: textColor) } } }
    private fun setupThreadSuccess(view: View, delivered: Boolean) { ItemThreadSuccessBinding.bind(view).apply { threadSuccess.setImageResource(if (delivered) R.drawable.ic_check_double_vector else org.fossify.messages.R.drawable.ic_check_vector); threadSuccess.applyColorFilter(conversationTheme()?.colors?.accent ?: textColor) } }
    ...
    private fun setupThreadSending(view: View) { ItemThreadSendingBinding.bind(view).threadSending.apply { setTextSize(TypedValue.COMPLEX_UNIT_PX, fontSize); setTextColor(conversationTheme()?.colors?.textSecondary ?: textColor) } }
