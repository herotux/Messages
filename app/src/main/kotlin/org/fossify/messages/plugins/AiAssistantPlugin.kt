...
/** Tool-based, OpenAI-compatible AI provider for the optional AI Assistant plugin. */
...
""".trimIndent()
    ...
    fun getConfig(context: Context): Config {
        ...
        val apiKey = if (!encrypted.isNullOrBlank()) decrypt(encrypted).orEmpty() else prefs.getString(KEY_API_KEY, "").orEmpty()
        return Config(
            ...
            systemPrompt = prefs.getString(KEY_SYSTEM, DEFAULT_SYSTEM_PROMPT).orEmpty().ifBlank { DEFAULT_SYSTEM_PROMPT }
        )
    }

    fun saveConfig(context: Context, config: Config) {
        val endpoint = config.endpoint.trim()
        require(endpoint.startsWith("https://", ignoreCase = true)) { "AI endpoint must use HTTPS" }
        require(config.model.trim().isNotBlank()) { "Model is required" }
        val editor = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit()
            .putString(KEY_ENDPOINT, endpoint)
            .putString(KEY_MODEL, config.model.trim())
            .putString(KEY_SYSTEM, config.systemPrompt.trim().ifBlank { DEFAULT_SYSTEM_PROMPT })
        val key = config.apiKey.trim()
        if (key.isBlank()) {
            editor.remove(KEY_API_KEY_ENCRYPTED).remove(KEY_API_KEY)
        } else {
            val encrypted = encrypt(key) ?: error("Secure storage is unavailable on this device")
            editor.putString(KEY_API_KEY_ENCRYPTED, encrypted).remove(KEY_API_KEY)
        }
        editor.apply()
    }

    /** Executes a bounded tool-calling loop. SMS tools execute locally on-device. */
    fun generate(context: Context, prompt: String, onResult: (Result<String>) -> Unit) {
        if (!isAvailable(context)) {
            onResult(Result.failure(IllegalStateException("AI Assistant is not active")))
            return
        }
        val config = getConfig(context)
        if (config.apiKey.isBlank()) {
            onResult(Result.failure(IllegalStateException("API key is not configured")))
            return
        }
        if (prompt.isBlank()) {
            onResult(Result.failure(IllegalArgumentException("Prompt is empty")))
            return
        }
        Thread {
            val result = runCatching { runAgent(context, config, prompt.trim()) }
            Handler(context.mainLooper).post { onResult(result) }
        }.start()
    }

    private fun runAgent(context: Context, config: Config, prompt: String): String {
        val messages = JSONArray()
            .put(JSONObject().put("role", "system").put("content", config.systemPrompt))
            .put(JSONObject().put("role", "user").put("content", prompt))

        for (round in 0 until MAX_TOOL_ROUNDS) {
            val response = postChat(config, messages)
            val message = response.optJSONArray("choices")?.optJSONObject(0)?.optJSONObject("message")
                ?: error("AI response has no message")
            ...
            for (i in 0 until calls.length()) {
                ...
                val toolResult = AiSmsToolEngine.execute(context, name, args)
        ...
        error("AI used too many tool steps")
    }

    private fun postChat(config: Config, messages: JSONArray): JSONObject {
        ...
        val payload = JSONObject()
            ...
            .put("tools", AiSmsToolEngine.definitions())
    ...
    private fun secretKey(): SecretKey? = runCatching {
        ...
        if (store.containsAlias(KEY_ALIAS)) return@runCatching store.getKey(KEY_ALIAS, null) as SecretKey
