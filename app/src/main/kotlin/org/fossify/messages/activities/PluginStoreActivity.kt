...
class PluginStoreActivity : SimpleActivity() {
    ...
    private fun addPlugin(plugin: PluginRegistry.Plugin) {
        ...
        if (!licensed) box.addView(Button(this).apply { text = "فعال‌سازی آزمایشی ۷ روزه"; setOnClickListener { PluginLicenseStore.activateTrial(this@PluginStoreActivity, plugin.id, 7); render() } })
        else {
            ...
            when (plugin.id) {
                ...
                PluginRegistry.SCHEDULED_SMS_PRO -> addInfo(box, "ارسال زمان‌بندی‌شده با AlarmManager اجرا می‌شود. پیام‌ها محلی ذخیره می‌شوند.")
            }
        }
    ...
    private fun addTemplates(parent: LinearLayout) {
        ...
        list.forEach { item -> parent.addView(TextView(this).apply { text = "• ${item.name}: ${item.body}"; textSize = 13f; setPadding(dp(4), dp(4), dp(4), dp(4)) }) }
    }
    ...
    private fun showAddRuleDialog() {
        ...
        com.google.android.material.dialog.MaterialAlertDialogBuilder(this).setTitle("قانون SMS Automation").setView(box).setNegativeButton("لغو", null).setPositiveButton("ذخیره") { _, _ ->
            SmsAutomationPlugin.addRule(this, SmsAutomationPlugin.Rule(System.currentTimeMillis(), name.text.toString().trim(), sender.text.toString().trim(), contains.text.toString().trim(), SmsAutomationPlugin.Action.MARK_READ, true)); SmsAutomationPlugin.setEnabled(this, true); render()
        }.show()
    }
    private fun showTemplateDialog() {
        val box = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(8), 0, dp(8), 0) }
        val name = field("نام قالب"); val body = EditText(this).apply { hint = "متن پیام؛ مثلا سلام {name}"; minLines = 3; gravity = Gravity.TOP; textSize = 15f }
