...
class BankCardsActivity : AppCompatActivity() {
    ...
    override fun onCreate(state: Bundle?) { super.onCreate(state); repository = BankCardsRepository(this); buildPage(); loadCards() }
    private fun buildPage() {
        val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; layoutDirection = View.LAYOUT_DIRECTION_RTL; setBackgroundColor(surfaceColor()); setPadding(dp(16), dp(8), dp(16), dp(16)) }
        val toolbar = MaterialToolbar(this).apply { title = "کارت‌های بانکی"; setTitleTextColor(onSurfaceColor()); setBackgroundColor(surfaceColor()); navigationIcon = getDrawable(androidx.appcompat.R.drawable.abc_ic_ab_back_material); setNavigationOnClickListener { finish() }; val more = menu.add("⋮"); more.setShowAsAction(2); setOnMenuItemClickListener { showTopMenu(); true } }
        root.addView(toolbar, LinearLayout.LayoutParams(-1, dp(64)))
        pager = ViewPager2(this).apply { orientation = ViewPager2.ORIENTATION_HORIZONTAL; clipToPadding = false; setPadding(dp(8), dp(12), dp(8), dp(12)); offscreenPageLimit = 2 }
        adapter = CardPagerAdapter(); pager.adapter = adapter
        root.addView(pager, LinearLayout.LayoutParams(-1, dp(300)))
        root.addView(TextView(this).apply { text = "برای دیدن کارت‌های دیگر، صفحه را بکشید"; gravity = Gravity.CENTER; textSize = 12f; setTextColor(onSurfaceVariant()); setPadding(0, 0, 0, dp(8)) })
        root.addView(MaterialButton(this).apply { text = "افزودن کارت جدید"; icon = getDrawable(android.R.drawable.ic_input_add); setOnClickListener { showWizard(null) } }, LinearLayout.LayoutParams(-1, dp(54)).apply { topMargin = dp(8) })
        setContentView(root)
    }
    private fun loadCards() { Thread { val result = repository.getCards(); runOnUiThread { cards = result.toMutableList(); pager.post { adapter.notifyDataSetChanged() } } }.start() }
    private inner class CardPagerAdapter : RecyclerView.Adapter<CardPagerAdapter.Holder>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = Holder(LinearLayout(parent.context).apply { orientation = LinearLayout.VERTICAL; layoutParams = RecyclerView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT); setPadding(dp(4)) })
        override fun onBindViewHolder(holder: Holder, position: Int) { holder.container.removeAllViews(); if (position < cards.size) holder.container.addView(createCard(cards[position]), LinearLayout.LayoutParams(-1, -1)) }
    }
    private fun createCard(card: BankCard): View { val visual = card.visual; val start = visual?.color ?: Color.rgb(58,72,90); return LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(22),dp(18),dp(22),dp(14)); background = GradientDrawable(GradientDrawable.Orientation.TL_BR,intArrayOf(start,darken(start))).apply { cornerRadius=dp(26).toFloat() }; elevation=dp(7).toFloat(); setOnClickListener{showCardActions(card)}; val top=LinearLayout(this@BankCardsActivity).apply{gravity=Gravity.CENTER_VERTICAL}; val logo=ImageView(this@BankCardsActivity).apply{visual?.logoResourceName?.let{name->resources.getIdentifier(name,"drawable",packageName).takeIf{it!=0}?.let(::setImageResource)}}; top.addView(logo,LinearLayout.LayoutParams(dp(42),dp(42))); top.addView(TextView(this@BankCardsActivity).apply{text=visual?.persianName?:card.bankId;textSize=15f;typeface=Typeface.DEFAULT_BOLD;setTextColor(Color.WHITE);setPadding(dp(10),0,0,0)},LinearLayout.LayoutParams(0,-2,1f)); top.addView(TextView(this@BankCardsActivity).apply{text="⋮";textSize=28f;gravity=Gravity.CENTER;setTextColor(Color.WHITE);setOnClickListener{showCardActions(card)}},LinearLayout.LayoutParams(dp(42),dp(44))); addView(top); addView(TextView(this@BankCardsActivity).apply{text=repository.formatCard(card.cardNumber);textSize=21f;typeface=Typeface.MONOSPACE;letterSpacing=.06f;gravity=Gravity.CENTER;textDirection=View.TEXT_DIRECTION_LTR;setTextColor(Color.WHITE);setPadding(0,dp(24),0,dp(12))}); addView(TextView(this@BankCardsActivity).apply{text=card.holderName.ifBlank{"نام صاحب کارت"};textSize=13f;typeface=Typeface.DEFAULT_BOLD;setTextColor(Color.WHITE)}); if(card.iban.isNotBlank()) addView(TextView(this@BankCardsActivity).apply{text=repository.formatIban(card.iban);textSize=11f;gravity=Gravity.CENTER;textDirection=View.TEXT_DIRECTION_LTR;setTextColor(Color.WHITE);setPadding(0,dp(6),0,0)}) } }
    ...
    private fun share(card:BankCard){val text=buildString{append(card.visual?.persianName?:"کارت بانکی").append('\n');append(repository.formatCard(card.cardNumber)).append('\n');if(card.holderName.isNotBlank())append(card.holderName).append('\n');if(card.iban.isNotBlank())append(repository.formatIban(card.iban))};startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply{type="text/plain";putExtra(Intent.EXTRA_TEXT,text)},"اشتراک‌گذاری"))}
    private fun showWizard(existing:BankCard?){wizard?.dismiss();wizard=CardWizard(existing).also{it.show()}}
    private inner class CardWizard(private val existing:BankCard?){private val dialog=BottomSheetDialog(this@BankCardsActivity);private val root=LinearLayout(this@BankCardsActivity).apply{orientation=LinearLayout.VERTICAL;layoutDirection=View.LAYOUT_DIRECTION_RTL;setPadding(dp(20),dp(12),dp(20),dp(28))};private val groups=Array(4){EditText(this@BankCardsActivity).apply{inputType=InputType.TYPE_CLASS_NUMBER;gravity=Gravity.CENTER;textSize=17f;hint="••••";maxLines=1}};private val holder=TextInputEditText(this@BankCardsActivity);private val iban=TextInputEditText(this@BankCardsActivity);private var preview:View?=null
        ...
        private fun header(title:String){root.removeAllViews();val row=LinearLayout(this@BankCardsActivity).apply{gravity=Gravity.CENTER_VERTICAL;layoutDirection=View.LAYOUT_DIRECTION_RTL};row.addView(TextView(this@BankCardsActivity).apply{text=title;textSize=20f;typeface=Typeface.DEFAULT_BOLD},LinearLayout.LayoutParams(0,-2,1f));row.addView(MaterialButton(this@BankCardsActivity).apply{text="اسکن کارت";icon=getDrawable(android.R.drawable.ic_menu_camera);setOnClickListener{scanner.launch(Intent(this@BankCardsActivity,BankCardScannerActivity::class.java).putExtra(BankCardScannerActivity.EXTRA_RETURN_TO_FORM,true))}});root.addView(row)}
        ...
        private fun save(){val card=groups.joinToString(""){repository.normalizeCard(it.text.toString())};val holderText=holder.text?.toString().orEmpty();val ibanText=repository.normalizeIban(iban.text?.toString().orEmpty());if(!repository.validCard(card)||repository.detect(card)==null||!repository.validIban(ibanText)){Toast.makeText(this@BankCardsActivity,"اطلاعات کارت معتبر نیست",Toast.LENGTH_SHORT).show();return};Thread{val result=repository.save(existing,card,holderText,ibanText);runOnUiThread{if(result.isSuccess){dialog.dismiss();wizard=null;hideKeyboard();loadCards()}else Toast.makeText(this@BankCardsActivity,"ذخیره کارت انجام نشد",Toast.LENGTH_SHORT).show()}}.start()}
