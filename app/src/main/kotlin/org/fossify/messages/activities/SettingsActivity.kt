...
class SettingsActivity : SimpleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupMaterialScrollListener(scrollingView = binding.settingsNestedScrollview, topAppBar = binding.settingsAppbar)
    }
    ...
    override fun onResume() {
        ...
        if (blockedNumbersAtPause != -1 && blockedNumbersAtPause != getBlockedNumbers().hashCode()) refreshConversations()
    ...
    private fun setupFoldersToggle() {
        val parent = binding.settingsHolder
        val existing = parent.findViewWithTag<LinearLayout>("folders_settings_section")
        val section = existing ?: LinearLayout(this).apply {
            tag = "folders_settings_section"
            orientation = LinearLayout.VERTICAL
            layoutParams = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT)
            setPadding(24, 20, 24, 8)

            val title = TextView(this@SettingsActivity).apply {
                text = if (Locale.getDefault().language == "fa") "مکالمات" else "Conversations"
            ...
            val switch = MaterialSwitch(this@SettingsActivity).apply {
                tag = "folders_settings_switch"
                text = if (Locale.getDefault().language == "fa") "نمایش پوشه‌های مکالمات" else "Show conversation folders"
                setPadding(0, 8, 0, 8)
                setOnClickListener {
                    ConversationFolderManager.setFoldersEnabled(this@SettingsActivity, isChecked)
                    refreshConversations()
                }
            }
            addView(switch, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT))

            val index = parent.indexOfChild(binding.settingsFontSizeHolder)
            parent.addView(this, index.coerceAtLeast(0).coerceAtMost(parent.childCount))
        }
        section.findViewWithTag<MaterialSwitch>("folders_settings_switch")?.isChecked = ConversationFolderManager.areFoldersEnabled(this)
    }
    ...
    private fun setupMessagesImport() { binding.settingsImportMessagesHolder.setOnClickListener { getContent.launch(messageImportFileTypes.toTypedArray()) } }

    override fun onPause() { super.onPause(); blockedNumbersAtPause = getBlockedNumbers().hashCode() }
    ...
    private fun setupLanguage() = binding.apply {
        ...
        if (isTiramisuPlus()) { settingsLanguageHolder.beVisible(); settingsLanguageHolder.setOnClickListener { launchChangeAppLanguageIntent() } }
    ...
    private fun setupManageBlockedNumbers() = binding.apply {
        ...
        settingsManageBlockedNumbersHolder.setOnClickListener {
            if (isOrWasThankYouInstalled()) startActivity(Intent(this@SettingsActivity, ManageBlockedNumbersActivity::class.java)) else FeatureLockedDialog(this@SettingsActivity) { }
        }
    }
    ...
    private fun setupManageBlockedKeywords() = binding.apply {
        ...
        settingsManageBlockedKeywordsHolder.setOnClickListener {
            if (isOrWasThankYouInstalled()) startActivity(Intent(this@SettingsActivity, ManageBlockedKeywordsActivity::class.java)) else FeatureLockedDialog(this@SettingsActivity) { }
        }
    }
    ...
    private fun setupSendLongMessageAsMMS() = binding.apply { settingsSendLongMessageMms.isChecked = config.sendLongMessageMMS; settingsSendLongMessageMmsHolder.setOnClickListener { settingsSendLongMessageMms.toggle(); config.sendLongMessageMMS = settingsSendLongMessageMms.isChecked } }
    private fun setupGroupMessageAsMMS() = binding.apply { settingsSendGroupMessageMms.isChecked = config.sendGroupMessageMMS; settingsSendGroupMessageMmsHolder.setOnClickListener { settingsSendGroupMessageMms.toggle(); config.sendGroupMessageMMS = settingsSendGroupMessageMms.isChecked } }
    ...
    private fun setupEmptyRecycleBin() = binding.apply {
        ...
        settingsEmptyRecycleBinHolder.setOnClickListener {
            if (recycleBinMessages == 0) toast(org.fossify.commons.R.string.recycle_bin_empty) else ConfirmationDialog(activity = this@SettingsActivity, message = "", messageId = R.string.empty_recycle_bin_messages_confirmation, positive = org.fossify.commons.R.string.yes, negative = org.fossify.commons.R.string.no) {
    ...
    private fun setupAppPasswordProtection() = binding.apply {
        ...
        settingsAppPasswordProtectionHolder.setOnClickListener {
            ...
            SecurityDialog(activity = this@SettingsActivity, requiredHash = config.appPasswordHash, showTabIndex = tabToShow) { hash, type, success ->
                if (success) {
                    ...
                    if (config.isAppPasswordProtectionOn) {
                        val confirmationTextId = if (config.appProtectionType == PROTECTION_FINGERPRINT) org.fossify.commons.R.string.fingerprint_setup_successfully else org.fossify.commons.R.string.protection_setup_successfully
                        ConfirmationDialog(activity = this@SettingsActivity, message = "", messageId = confirmationTextId, positive = org.fossify.commons.R.string.ok, negative = 0) { }
                    }
                }
            }
        }
