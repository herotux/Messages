...
class ThemeBuilderActivity : SimpleActivity() {
    ...
    private fun renderThemeLibrary() {
        ...
        listOf(t("همه", "All"), t("ساخته من", "My themes"), t("واردشده", "Imported"), t("پسندیده", "Favorites")).forEachIndexed { index, label ->
            filterRow.addView(actionButton(label) { themeFilter = index; renderThemeCards(content, search.text.toString()) }, LinearLayout.LayoutParams(0, dp(42), 1f).apply { if (index > 0) marginStart = dp(5) })
        }
        content.addView(filterRow, LinearLayout.LayoutParams(-1, dp(46)).apply { topMargin = dp(8) })
        search.addTextChangedListener(object : android.text.TextWatcher {
            override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) = Unit
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) { renderThemeCards(content, s?.toString().orEmpty()) }
    ...
    private fun renderThemeCards(content: LinearLayout, query: String) {
        ...
        val themes = when (themeFilter) {
            1 -> all.filter { it.source == ThemeManager.ThemeSource.USER }
            2 -> all.filter { it.source == ThemeManager.ThemeSource.IMPORTED }
            3 -> all.filter { ThemeManager.isFavorite(this, it.id) }
            else -> all
        }
        val groups = listOf(
            ThemeManager.ThemeSource.BUILT_IN to t("تم‌های آماده", "Built-in themes"),
            ThemeManager.ThemeSource.USER to t("تم‌های من", "My themes"),
            ThemeManager.ThemeSource.IMPORTED to t("تم‌های واردشده", "Imported themes"),
            ThemeManager.ThemeSource.COMMUNITY to t("تم‌های جامعه", "Community themes")
        )
        var count = 0
        groups.forEach { (source, title) ->
            val group = themes.filter { it.source == source }
            if (group.isEmpty()) return@forEach
            content.addView(sectionTitle(title), LinearLayout.LayoutParams(-1, -2).apply { topMargin = dp(16) })
            group.forEach { theme -> content.addView(themeCard(theme), LinearLayout.LayoutParams(-1, -2).apply { topMargin = dp(8) }); count++ }
        }
    ...
    private fun themeCard(theme: ThemeManager.ThemeDefinition): MaterialCardView = MaterialCardView(this).apply {
        ...
        strokeColor = if (selected) theme.colors.accent else attrColor(com.google.android.material.R.attr.colorOutlineVariant)
        ...
        header.addView(TextView(this@ThemeBuilderActivity).apply {
            text = "◉"
            textSize = 20f
            gravity = Gravity.CENTER
            setTextColor(theme.colors.accent)
            contentDescription = t("پیش‌نمایش", "Preview")
            setOnClickListener { openPreview(theme) }
        }, LinearLayout.LayoutParams(dp(44), dp(40)))
        header.addView(TextView(this@ThemeBuilderActivity).apply {
            text = if (ThemeManager.isFavorite(this@ThemeBuilderActivity, theme.id)) "★" else "☆"
            textSize = 24f
            gravity = Gravity.CENTER
            setTextColor(theme.colors.accent)
            setOnClickListener { ThemeManager.toggleFavorite(this@ThemeBuilderActivity, theme.id); renderThemeLibrary() }
        }, LinearLayout.LayoutParams(dp(44), dp(40)))
        box.addView(header)
        ...
        box.addView(preview)
        val source = when (theme.source) {
            ThemeManager.ThemeSource.BUILT_IN -> t("آماده", "Built-in")
            ThemeManager.ThemeSource.USER -> t("ساخته من", "My theme")
            ThemeManager.ThemeSource.IMPORTED -> t("واردشده", "Imported")
            ThemeManager.ThemeSource.COMMUNITY -> t("جامعه", "Community")
        }
    ...
    private fun showThemeActions(theme: ThemeManager.ThemeDefinition) {
        ...
        MaterialAlertDialogBuilder(this).setTitle(if (english()) theme.nameEn else theme.nameFa).setItems(items) { _, which ->
            if (builtIn) when (which) {
                0 -> { ThemeManager.select(this, theme.id); renderThemeLibrary() }
                1 -> openPreview(theme)
                2 -> duplicateTheme(theme)
                3 -> { ThemeManager.toggleFavorite(this, theme.id); renderThemeLibrary() }
            } else when (which) {
                0 -> { ThemeManager.select(this, theme.id); renderThemeLibrary() }
                1 -> openPreview(theme)
                2 -> openEditor(theme.id)
                3 -> duplicateTheme(theme)
                4 -> exportThemeFile(theme)
                5 -> shareThemeFile(theme)
                6 -> { ThemeManager.toggleFavorite(this, theme.id); renderThemeLibrary() }
                7 -> confirmDeleteTheme(theme)
            }
        }.show()
    }

    private fun duplicateTheme(theme: ThemeManager.ThemeDefinition) {
        val copy = theme.copy(id = "user_${UUID.randomUUID()}", source = ThemeManager.ThemeSource.USER, nameFa = theme.nameFa + " (کپی)", nameEn = theme.nameEn + " (Copy)")
        ThemeManager.saveUserTheme(this, copy)
        ThemeManager.select(this, copy.id)
        renderThemeLibrary()
    }
    ...
    private fun confirmDeleteTheme(theme: ThemeManager.ThemeDefinition) {
        MaterialAlertDialogBuilder(this).setTitle(t("حذف تم؟", "Delete theme?")).setMessage(t("تم «${theme.nameFa}» حذف شود؟", "Delete “${theme.nameEn}”?"))
            .setNegativeButton(t("لغو", "Cancel"), null).setPositiveButton(t("حذف", "Delete")) { _, _ -> ThemeManager.deleteCustomTheme(this, theme.id); renderThemeLibrary() }.show()
    }
    ...
    private fun shareThemeFile(theme: ThemeManager.ThemeDefinition) {
        val file = runCatching { java.io.File(cacheDir, "${theme.id}${ThemeFileManager.FILE_EXTENSION}").apply { writeText(ThemeFileManager.export(this@ThemeBuilderActivity, theme), Charsets.UTF_8) } }.getOrNull()
            ?: return showThemeError("ساخت فایل اشتراک‌گذاری انجام نشد")
        val uri = androidx.core.content.FileProvider.getUriForFile(this, "${BuildConfig.APPLICATION_ID}.provider", file)
        startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply { type = ThemeFileManager.MIME_TYPE; putExtra(Intent.EXTRA_STREAM, uri); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION) }, t("اشتراک‌گذاری تم", "Share theme")))
    }
    ...
    private fun showGradientDialog(start: Int, end: Int, angle: Int, onApply: (Int, Int, Int) -> Unit) {
        ...
        val layout = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(8), dp(4), dp(8), 0) }; layout.addView(startInput); layout.addView(endInput); layout.addView(TextView(this).apply { text = "زاویه گرادیان"; setPadding(0, dp(8), 0, dp(2)) }); layout.addView(spinner)
        MaterialAlertDialogBuilder(this).setTitle("تنظیم گرادیان").setView(layout).setPositiveButton("اعمال") { _, _ -> runCatching { Color.parseColor(startInput.text.toString().trim()) to Color.parseColor(endInput.text.toString().trim()) }.onSuccess { (a, b) -> onApply(a, b, angles[spinner.selectedItemPosition]) }.onFailure { showThemeError("رنگ گرادیان نامعتبر است") } }.setNegativeButton("لغو", null).show()
    }
