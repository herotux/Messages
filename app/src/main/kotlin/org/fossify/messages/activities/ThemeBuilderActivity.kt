...
/** First version of the user Theme Builder: editable palette, live preview and persistence. */
class ThemeBuilderActivity : SimpleActivity() {
    ...
    private fun build(existing: ThemeManager.ThemeDefinition?) {
        ...
        val p = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(16), dp(16), dp(16), dp(16)) }
        ...
        p.addView(previewTitle)
        p.addView(previewIncoming, LinearLayout.LayoutParams(-1, -2).apply { setMargins(0, dp(12), dp(28), dp(6)) })
        p.addView(previewOutgoing, LinearLayout.LayoutParams(-1, -2).apply { setMargins(dp(28), dp(6), 0, 0) })
        preview.addView(p)
        ...
        fields.forEach { field ->
            ...
            val label = TextView(this).apply { text = field.fa; textSize = 15f }
            row.addView(label, LinearLayout.LayoutParams(0, dp(52), 1f))
            val swatch = View(this).apply { background = android.graphics.drawable.GradientDrawable().apply { shape = android.graphics.drawable.GradientDrawable.RECTANGLE; cornerRadius = dp(12).toFloat(); setColor(field.color) } }
        ...
        root.addView(save, LinearLayout.LayoutParams(-1, dp(52)).apply { setMargins(0, dp(16), 0, 0) })
        setContentView(root)
        updatePreview()
    }
    ...
    private fun chooseColor(field: Field, swatch: View) {
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton("اعمال") { _, _ ->
                ...
                if (parsed != null) { field.color = parsed; swatch.background = android.graphics.drawable.GradientDrawable().apply { cornerRadius = dp(12).toFloat(); setColor(parsed) }; updatePreview() }
            }
    ...
    private fun updatePreview() {
        ...
        previewIncoming.setTextColor(get("text")); previewIncoming.backgroundTintList = android.content.res.ColorStateList.valueOf(get("incoming"))
        previewOutgoing.setTextColor(if (isLight(get("outgoing"))) Color.BLACK else Color.WHITE); previewOutgoing.backgroundTintList = android.content.res.ColorStateList.valueOf(get("outgoing"))
    }
