...
class ThemeBuilderActivity : SimpleActivity() {
    ...
    private fun shareThemeFile(theme: ThemeManager.ThemeDefinition) {
        val file = runCatching { java.io.File(cacheDir, "${theme.id}${ThemeFileManager.FILE_EXTENSION}").apply { writeText(ThemeFileManager.export(theme), Charsets.UTF_8) } }.getOrNull() ?: return showThemeError("ساخت فایل اشتراک‌گذاری انجام نشد")
