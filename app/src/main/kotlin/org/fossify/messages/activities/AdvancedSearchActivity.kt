...
class AdvancedSearchActivity : SimpleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setBackgroundColor(getProperBackgroundColor()) }
        ...
        fromButton = MaterialButton(this).apply { text = "From date" }; toButton = MaterialButton(this).apply { text = "To date" }
        dates.addView(fromButton, LinearLayout.LayoutParams(0, dp(52), 1f)); dates.addView(toButton, LinearLayout.LayoutParams(0, dp(52), 1f)); content.addView(dates)
    ...
    private fun field(hint: String, input: TextInputEditText) = TextInputLayout(this).apply { this.hint = hint; addView(input); layoutParams = LinearLayout.LayoutParams(-1, dp(64)) }
    ...
    private fun runSearch() {
        ...
        Thread { val hits = searcher.search(filter); runOnUiThread { render(hits) } }.start()
    }
    ...
    private fun openThread(hit: AdvancedSearchHit) { startActivity(Intent(this, ThreadActivity::class.java).apply { putExtra(THREAD_ID, hit.threadId); putExtra(SEARCHED_MESSAGE_ID, hit.messageId) }) }
