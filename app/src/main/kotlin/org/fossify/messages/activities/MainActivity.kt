...
class MainActivity : SimpleActivity() {
    ...
    override fun onResume() {
        ...
        binding.mainMenu.requireToolbar().navigationIcon?.setTint(getProperTextColor())
        checkShortcut()
    }
    ...
    override fun onBackPressedCompat(): Boolean {
        return when {
            binding.mainDrawer.isOpen -> { binding.mainDrawer.close(); true }
            binding.mainMenu.isSearchOpen -> { binding.mainMenu.closeSearch(); true }
    ...
    private fun setupOptionsMenu() {
        ...
        toolbar.setNavigationIcon(AppCompatResources.getDrawable(this, R.drawable.ic_menu_material))
        toolbar.navigationIcon?.setTint(getProperTextColor())
        toolbar.setNavigationOnClickListener { binding.mainDrawer.open() }
        ...
        binding.mainNavigationDrawer.setNavigationItemSelectedListener { item ->
            val handled = when (item.itemId) {
                R.id.show_recycle_bin -> { launchRecycleBin(); true }
                R.id.show_archived -> { launchArchivedConversations(); true }
                R.id.settings -> { launchSettings(); true }
                R.id.about -> { launchAbout(); true }
                else -> false
            }
            if (handled) binding.mainDrawer.close()
            handled
        }
    }

    private fun refreshMenuItems() {
        binding.mainNavigationDrawer.menu.apply {
    ...
    private fun updateMenuColors() {
        ...
        binding.mainNavigationDrawer.setBackgroundColor(getProperBackgroundColor())
    }
    ...
    private fun loadMessages() {
        if (isQPlus()) {
            ...
            if (Telephony.Sms.getDefaultSmsPackage(this) == packageName) askPermissions()
            else {
            ...
            if (it) handlePermission(PERMISSION_SEND_SMS) {
                if (it) handlePermission(PERMISSION_READ_CONTACTS) {
                    handleNotificationPermission { granted ->
                        if (!granted) PermissionRequiredDialog(activity = this, textId = org.fossify.commons.R.string.allow_notifications_incoming_messages, positiveActionCallback = { openNotificationSettings() })
                    }
                    initMessenger()
    ...
    private fun getCachedConversations() {
        ensureBackgroundThread {
            ...
            runOnUiThread { setupConversations(conversations, cached = true); getNewConversations((conversations + archived).toMutableList() as ArrayList<Conversation>) }
    ...
    private fun getNewConversations(cachedConversations: ArrayList<Conversation>) {
        ensureBackgroundThread {
            ...
            if (config.appRunCount == 1) conversations.map { it.threadId }.forEach { threadId ->
                getMessages(threadId, includeScheduledMessages = false).chunked(30).forEach { currentMessages -> messagesDB.insertMessages(*currentMessages.toTypedArray()) }
            }
        }
    }
    ...
    private fun notifyDatasetChanged() { getOrCreateConversationsAdapter().notifyDataSetChanged() }

    private fun handleConversationClick(any: Any) {
        Intent(this, ThreadActivity::class.java).apply { val conversation = any as Conversation; putExtra(THREAD_ID, conversation.threadId); putExtra(THREAD_TITLE, conversation.title); startActivity(this) }
    }
    ...
    private fun checkShortcut() {
        ...
        if (config.lastHandledShortcutColor != appIconColor) {
            ...
            try { manager.dynamicShortcuts = listOf(newConversation); config.lastHandledShortcutColor = appIconColor } catch (_: Exception) { }
        }
    }
    ...
    private fun getCreateNewContactShortcut(appIconColor: Int): ShortcutInfo {
        ...
        val intent = Intent(this, NewConversationActivity::class.java).apply { action = Intent.ACTION_VIEW }
        return ShortcutInfo.Builder(this, "new_conversation").setShortLabel(newEvent).setLongLabel(newEvent).setIcon(Icon.createWithBitmap(bmp)).setIntent(intent).setRank(0).build()
    }
    ...
    private fun searchTextChanged(text: String, forceUpdate: Boolean = false) {
        ...
        } else { binding.searchPlaceholder.beVisible(); binding.searchResultsList.beGone() }
    }

    private fun showSearchResults(messages: List<Message>, conversations: List<Conversation>, searchedText: String) {
        val searchResults = ArrayList<SearchResult>()
        conversations.forEach { conversation ->
            val date = (conversation.date * 1000L).formatDateOrTime(context = this, hideTimeOnOtherDays = true, showCurrentYear = true)
            searchResults.add(SearchResult(messageId = -1, title = conversation.title, snippet = conversation.phoneNumber, date = date, threadId = conversation.threadId, photoUri = conversation.photoUri))
        }
        messages.sortedByDescending { it.id }.forEach { message ->
            ...
            val date = (message.date * 1000L).formatDateOrTime(context = this, hideTimeOnOtherDays = true, showCurrentYear = true)
            searchResults.add(SearchResult(messageId = message.id, title = recipient, snippet = message.body, date = date, threadId = message.threadId, photoUri = message.senderPhotoUri))
        }
        runOnUiThread {
            ...
            if (currAdapter == null) {
                SearchResultsAdapter(this, searchResults, binding.searchResultsList, searchedText) {
                    hideKeyboard()
                    Intent(this, ThreadActivity::class.java).apply { putExtra(THREAD_ID, (it as SearchResult).threadId); putExtra(THREAD_TITLE, it.title); putExtra(SEARCHED_MESSAGE_ID, it.messageId); startActivity(this) }
                }.apply { binding.searchResultsList.adapter = this }
    ...
    private fun launchAbout() {
        ...
        val faqItems = arrayListOf(
            ...
            FAQItem(title = org.fossify.commons.R.string.faq_9_title_commons, text = org.fossify.commons.R.string.faq_9_text_commons)
        )
        if (!resources.getBoolean(org.fossify.commons.R.bool.hide_google_relations)) {
            faqItems.add(FAQItem(title = org.fossify.commons.R.string.faq_2_title_commons, text = org.fossify.commons.R.string.faq_2_text_commons))
            faqItems.add(FAQItem(title = org.fossify.commons.R.string.faq_6_title_commons, text = org.fossify.commons.R.string.faq_6_text_commons))
        }
    ...
    fun refreshConversations(@Suppress("unused") event: Events.RefreshConversations) { initMessenger() }
