...
object ThemeManager {
    ...
    fun selectedThemeId(context: Context): String = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getString(KEY_THEME_ID, DEFAULT_ID) ?: DEFAULT_ID
    ...
    fun select(context: Context, id: String): Boolean { if (allThemes(context).none { it.id == id }) return false; context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString(KEY_THEME_ID, id).apply(); return true }
    fun selectBuiltIn(context: Context, id: String): Boolean { if (builtInThemes.none { it.id == id }) return false; return select(context, id) }
    ...
    fun setFavorite(context: Context, id: String, favorite: Boolean) { if (find(context, id) == null) return; val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE); val favorites = prefs.getStringSet(KEY_FAVORITES, emptySet())?.toMutableSet() ?: mutableSetOf(); if (favorite) favorites.add(id) else favorites.remove(id); prefs.edit().putStringSet(KEY_FAVORITES, favorites).apply() }
