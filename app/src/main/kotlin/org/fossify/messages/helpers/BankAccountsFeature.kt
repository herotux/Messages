...
object BankAccountsFeature {
    ...
    fun showBankAccountManager(context: Context) {
        ...
        val add = Button(context).apply { text = title(context, "افزودن کارت", "Add bank card"); setOnClickListener { showEditDialog(context, null) { refreshManager(context, cardsHolder, dialog) } } }
        val scan = Button(context).apply { text = title(context, "اسکن کارت", "Scan card"); setOnClickListener { if (context is Activity) context.startActivity(Intent(context, BankCardScannerActivity::class.java)) else Toast.makeText(context, title(context, "امکان باز کردن دوربین وجود ندارد", "Cannot open camera"), Toast.LENGTH_SHORT).show() } }
        actions.addView(add, LinearLayout.LayoutParams(0, -2, 1f)); actions.addView(scan, LinearLayout.LayoutParams(0, -2, 1f))
    ...
    private fun refreshManager(context: Context, holder: LinearLayout, dialog: BottomSheetDialog? = null) {
        ensureBackgroundThread {
            ...
            context.runOnUiThreadCompat {
                ...
                if (accounts.isEmpty()) {
                    holder.addView(TextView(context).apply { text = title(context, "هنوز کارتی اضافه نشده است", "No bank cards added yet"); gravity = Gravity.CENTER; setTextColor(Color.GRAY) }, LinearLayout.LayoutParams(dp(context, 260), -1))
    ...
    private fun createCardView(context: Context, account: BankAccount, click: () -> Unit): View {
        ...
        val card = LinearLayout(context).apply {
            ...
            setPadding(dp(context, 18), dp(context, 14), dp(context, 18), dp(context, 12))
        ...
        top.addView(TextView(context).apply { text = bank?.let { title(context, it.persianName, it.englishName) } ?: account.bankId; textSize = 15f; setTypeface(typeface, Typeface.BOLD); setTextColor(Color.WHITE); setPadding(dp(context, 10), 0, 0, 0) })
        card.addView(top)
    ...
    private fun showEditDialog(context: Context, existing: BankAccount?, onSaved: () -> Unit) {
        ...
        dialog.setOnShowListener {
            dialog.getButton(android.content.DialogInterface.BUTTON_POSITIVE).setOnClickListener {
                ...
                ensureBackgroundThread {
                    try { save(context, existing, bank.id.name, normalizedCard, holder.text.toString(), normalizedIban); context.runOnUiThreadCompat { dialog.dismiss(); onSaved() } }
                    catch (_: Exception) { context.runOnUiThreadCompat { Toast.makeText(context, title(context, "ذخیره کارت انجام نشد", "Could not save bank card"), Toast.LENGTH_SHORT).show() } }
                }
            }
        }
    ...
    fun showCardPicker(context: Context, onSelected: (BankAccount) -> Unit) {
        ...
        root.addView(TextView(context).apply { text = title(context, "شماره کارت با دکمه کپی یا QR قابل استفاده است", "Use Copy or QR on a saved card"); textSize = 12f; setTextColor(Color.GRAY); setPadding(0, 0, 0, dp(context, 8)) })
        ...
        scroll.addView(holder, ViewGroup.LayoutParams(-1, dp(context, 220))); root.addView(scroll); dialog.setContentView(root); dialog.show()
    ...
    fun addPickerButton(activity: Activity) {
        ...
        val button = LinearLayout(activity).apply {
            tag = "bank_card_picker_button"; orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER; isClickable = true; background = ContextCompat.getDrawable(activity, R.drawable.ripple_background); setPadding(dp(activity, 12), dp(activity, 12), dp(activity, 12), dp(activity, 12))
            addView(TextView(activity).apply { text = "💳"; textSize = 26f; gravity = Gravity.CENTER }, LinearLayout.LayoutParams(-1, dp(activity, 42)))
            ...
            setOnClickListener { showCardPicker(activity) { account -> val edit = activity.findViewById<EditText>(R.id.thread_type_message); if (edit != null) { val text = buildString { append(title(activity, "شماره کارت", "Card number")); append(": "); append(formatCard(account.cardNumber)); if (account.holderName.isNotBlank()) { append("\n"); append(title(activity, "به نام", "Name")); append(": "); append(account.holderName) }; if (account.iban.isNotBlank()) { append("\n"); append("IBAN: "); append(formatIban(account.iban)) } }; val start = edit.selectionStart.coerceAtLeast(0); val end = edit.selectionEnd.coerceAtLeast(0); edit.text.replace(minOf(start, end), max(start, end), text); edit.setSelection((minOf(start, end) + text.length).coerceAtMost(edit.length())) } } }
        }
        picker.addView(button, ViewGroup.LayoutParams(dp(activity, 96), dp(activity, 112)))
        val flow = picker.getChildAt(0)
        if (flow is androidx.constraintlayout.helper.widget.Flow) { val ids = flow.referencedIds.toMutableList(); ids.add(button.id.takeIf { it != View.NO_ID } ?: View.generateViewId().also { button.id = it }); flow.referencedIds = ids.toIntArray() }
    }
    ...
    fun installMessageCardLinks(activity: Activity) {
        ...
        val listener = object : androidx.recyclerview.widget.RecyclerView.OnChildAttachStateChangeListener { override fun onChildViewAttachedToWindow(view: View) { decorateTree(view, activity) }; override fun onChildViewDetachedFromWindow(view: View) = Unit }
        if (list.getTag(R.id.bank_card_feature_tag) != true) { list.addOnChildAttachStateChangeListener(listener); list.setTag(R.id.bank_card_feature_tag, true) }
    ...
    private fun decorateTextView(view: TextView, activity: Activity) {
        val original = view.text?.toString() ?: return; val matches = cardRegex.findAll(original).toList(); if (matches.isEmpty()) return
        ...
        view.text = spannable; view.movementMethod = LinkMovementMethod.getInstance(); view.highlightColor = Color.TRANSPARENT
    }

    private fun showDetectedCard(activity: Activity, card: String, message: String) {
        val bank = IranianBankRegistry.findByCard(card); val iban = ibanRegex.find(normalizeDigits(message))?.value; val holder = holderRegex.find(message)?.groupValues?.getOrNull(1)?.trim()
        ...
        val dialog = BottomSheetDialog(activity); dialog.setContentView(root); dialog.show()
    }
    ...
    private fun shareQr(context: Context, account: BankAccount, iban: Boolean) {
        ...
        try {
            ...
            val file = File(dir, "bank_${account.id}_${if (iban) "iban" else "card"}.png")
            FileOutputStream(file).use { bitmap.compress(Bitmap.CompressFormat.PNG, 100, it) }
            val uri: Uri = FileProvider.getUriForFile(context, "${context.packageName}.provider", file)
            val share = Intent(Intent.ACTION_SEND).apply { type = "image/png"; putExtra(Intent.EXTRA_STREAM, uri); putExtra(Intent.EXTRA_TEXT, payload); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION) }
            context.startActivity(Intent.createChooser(share, title(context, "اشتراک‌گذاری QR", "Share QR")))
        } catch (_: Exception) { Toast.makeText(context, title(context, "اشتراک‌گذاری انجام نشد", "Could not share QR"), Toast.LENGTH_SHORT).show() }
    }
