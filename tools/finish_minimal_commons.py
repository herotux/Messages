...
binding_classes = set()
for text in texts:
    binding_classes.update(re.findall(r"\b([A-Z][A-Za-z0-9_]*)Binding\b", text))


def binding_to_layout(cls):
    # The regex above captures the class name without the Binding suffix.
    base = cls
...
# Collect resource references from every vendored Commons source and copied layouts.
need = set()
...
while queue:
            ...
            try:
                ...
                for child in children:
                    if key(child) == (typ, name):
                        queue.extend(refs(ET.tostring(child, encoding="unicode")))
            except Exception:
                pass
        else:
            try:
    ...
    if typ == "id":
        for src in RES.rglob("*"):
            if not src.is_file() or is_bank(src) or not src.parent.name.startswith("layout"):
                continue
            try:
                xml = read(src)
            except Exception:
...
def cleanup_duplicate_values():
    ...
    for values_dir in res_root.glob('values*'):
        ...
        for key, entries in resources.items():
            if len(entries) < 2:
                continue
            preferred = [e for e in entries if e[0].name != 'strings.xml']
            keep_path = preferred[0][0] if preferred else entries[0][0]
